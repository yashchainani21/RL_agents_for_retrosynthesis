...
def perform_DORAnet_chem_1step(precursor_smiles: str):
    ...
    forward_network = synthetic.generate_network(
        ...
        helpers = ("O", "O=O", "[H][H]", "O=C=O", "C=O", "[C-]#[O+]", "Br", "[Br][Br]", "CO", "[C-]#[O+]", "C=C",
...
def process_precursors_parallel(precursors, num_workers=mp.cpu_count()):
    ...
    with mp.Pool(processes=num_workers) as pool:
        results = pool.map(perform_DORAnet_chem_1step, precursors)

    # Flatten results (convert list of lists into a single list)
    all_chem_products_list = [prod for sublist in results for prod in sublist]

    return all_chem_products_list

if __name__ == "__main__":

    print(f"Running with {mp.cpu_count()} cores...")
    all_chem_products_list = process_precursors_parallel(precursors_list, num_workers = mp.cpu_count())
    all_chem_products_list = list(set(all_chem_products_list))
    print(f"\nNumber of total, unique bioproducts generated: {len(all_chem_products_list)}\n")

    # save all chemically modified products
    with open(output_filepath, 'w') as output_file:
        output_file.write('\n'.join(all_chem_products_list))
